...
import { AppFlowMockImplementation } from '#src/business/service/__mocks__/app-flow-mock-implementation.js'
import { type LifeCycleMockImplementation } from '#src/business/service/__mocks__/life-cycle-mock-implementation.js'
import { AppFlow, FlowDirectionMapper } from '#src/business/service/app-flow.js'
...
describe('AppFlow', () => {
	...
	describe('destroy', () => {
		it('should call _deepExecFlowList and _topLevelReversedFlowList', async () => {
			...
			const spy_deepExecFlowList = vi.spyOn(AppFlow, 'DeepExecFlowList').mockImplementation(() => Promise.resolve())
			// eslint-disable-next-line @typescript-eslint/no-explicit-any
			const mockedAppFlow = appFlow as any
			const spy_topLevelReversedFlowList = vi
				.spyOn(mockedAppFlow, '_topLevelReversedFlowList')
				.mockReturnValue(reversFlow)

			await appFlow.destroy()
			expect(spy_deepExecFlowList).toHaveBeenCalledTimes(1)
			expect(spy_deepExecFlowList).toHaveBeenCalledWith({
